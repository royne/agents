...
import { AdSummary } from '../../services/data-analysis/DailyOrdersUTMService';

interface UTMAdsChartProps {
  ads: Record<string, AdSummary>;
  limit?: number;
  title?: string;
}
...
const UTMAdsChart: React.FC<UTMAdsChartProps> = ({ ads, limit = 15, title = 'Anuncios (UTM content)' }) => {
  const data = useMemo<Row[]>(() => {
    const rows = Object.entries(ads).map(([name, summary]) => {
