...
const PhoneMockup: React.FC<PhoneMockupProps> = ({ landingState, className }) => {
  ...
  return (
    <div className={className || "flex flex-col items-center"}>
      <div className="w-[300px] h-[600px] bg-black rounded-[50px] p-2 border-[6px] border-white/10 shadow-[0_50px_100px_rgba(0,0,0,0.8)] relative overflow-hidden group/mockup">
        ...
        {!hasScrolled && (landingState.proposedStructure?.sections?.length || 0) > 2 && (
                ...
                <div key={s.sectionId} className="w-full relative border-b border-white/5 last:border-0 overflow-hidden group/section">
                  {generation?.imageUrl ? (
                    <img
                      ...
                      alt={s.title}
                      className="w-full h-auto object-cover min-h-[120px] transition-transform duration-700 group-hover/section:scale-110"
                    />
                  ) : (
